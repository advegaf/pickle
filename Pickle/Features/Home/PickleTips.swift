...
/// First-run contextual tips, shown one at a time in order (TipKit persists what has
/// been seen). Gated behind onboarding via a shared parameter so nothing pops during
/// setup. Reset alongside "Delete all my data" (Tips.resetDatastore in AboutView).
enum PickleTipsGate {
