...
struct MainTabView: View {
    ...
    /// First-run coach marks, shown one at a time across Home + the bar; each advances
    /// only via its X (custom bubbles - TipKit popovers could not be made X-only).
    @StateObject private var coachMarks = CoachMarks()
